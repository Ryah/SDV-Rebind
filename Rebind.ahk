...
; Startup
Menu, Tray, Icon, ico/ico.ico

; Check for initial config
if FileExist("config.ini") {
    goto Init
}
Msgbox, Config not found. Please first run the Control Panel.
ExitApp
...
Init:
    ...
    if (update = 1) {
        WindowID:=Notify.AddWindow("Settings Updated",{Size:30,Radius:15,Time:3000,Font:"Consolas",Color:"0xf8f8ff",Background:"0x301934",Animate:"Bottom,Slide",Destroy:"Bottom,Slide",ShowDelay:250,Title:"SDV Rebind"})
...
#If WinActive("ahk_exe Stardew Valley.exe")
    ...
    if (acChkBox = 1) {
        Hotkey, %acKey%, ACRebind
    }
return

WSCRebind:
    ; Msgbox, WSC
    SendInput, {LAlt down}{Enter down}{e down}
    KeyWait, %wscKey%
    SendInput, {LAlt up}{Enter up}{e up}
return
...
UpdateKeys:
    ...
    if (updated = 1) {
        IniWrite, 1, config.ini, system, keysUpdated
        updated := 0
        Reload
    }
return
