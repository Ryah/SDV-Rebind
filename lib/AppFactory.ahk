...
Class AppFactory {
	...
	class _IOControl {
		...
		__New(parent, guid, options, callback){
			...
			Menu, % this.id, Add, % "SuppressRepeats", % fn
		...
		IOControlChoiceMade(val){
			if (val == 1){
				...
				; SuppressRepeats Repeats
				this.BindObject.BindOptions.SuppressRepeats := !this.BindObject.BindOptions.SuppressRepeats
				this.SetMenuCheckState("SuppressRepeats")
				this.BindModeEnded(this.BindObject)
			} else if (val == 5){
				; Clear
				this.BindObject := new this.parent._BindObject()
	...
	InputEvent(ControlGUID, e){
		; SuppressRepeats
		if (this.IOControls[ControlGuid].BindObject.BindOptions.SuppressRepeats && (this.IOControls[ControlGuid].State == e))
			return
